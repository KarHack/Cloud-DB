...
public class Result {
    ...
    // The Method with Some Authentication to be used only for a Single Row.
    public static JSONObject getRowAsJSONObj(ResultSet rowResultSet) {
	try {
	    // Now we will Try to Extract the Column Data from the ResultSet.
	    // Check if the Result set is set to be read.
	    if (rowResultSet.isBeforeFirst()) {
		rowResultSet.next();
	    }
	    // Now lets extract the data.
	    JSONObject rowDataJObj = new JSONObject();
	    ResultSetMetaData resultSetMeta = rowResultSet.getMetaData();
		...
		try {
			...
			case java.sql.Types.ARRAY:
			    rowDataJObj.put(columnName, rowResultSet.getArray(columnName));
			    break;
			case java.sql.Types.BOOLEAN:
			    rowDataJObj.put(columnName, rowResultSet.getBoolean(columnName));
			    break;
			case java.sql.Types.BLOB:
			    rowDataJObj.put(columnName, rowResultSet.getBlob(columnName));
			    break;
			case java.sql.Types.BIGINT:
			    rowDataJObj.put(columnName, rowResultSet.getLong(columnName));
			    break;
			case java.sql.Types.DOUBLE:
			    rowDataJObj.put(columnName, rowResultSet.getDouble(columnName));
			    break;
			case java.sql.Types.FLOAT:
			    rowDataJObj.put(columnName, rowResultSet.getFloat(columnName));
			    break;
			case java.sql.Types.INTEGER:
			    rowDataJObj.put(columnName, rowResultSet.getInt(columnName));
			    break;
			case java.sql.Types.NVARCHAR:
			    rowDataJObj.put(columnName, rowResultSet.getNString(columnName));
			    break;
			case java.sql.Types.VARCHAR:
			    rowDataJObj.put(columnName, rowResultSet.getString(columnName));
			    break;
			...
			case java.sql.Types.DATE:
			    rowDataJObj.put(columnName, rowResultSet.getDate(columnName).toString());
			    break;
			case java.sql.Types.TIMESTAMP:
			    rowDataJObj.put(columnName, rowResultSet.getTimestamp(columnName).toString());
			    break;
			default:
			    rowDataJObj.put(columnName, rowResultSet.getObject(columnName).toString());
			    break;
		    }
		} catch (Exception e) {
		    // There was an Error.
