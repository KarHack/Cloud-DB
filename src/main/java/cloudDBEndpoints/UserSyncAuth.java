 ...
 * @author karan
 */
public class UserSyncAuth extends HttpServlet {
	...
	try {
		...
		out.println("Can only GET this API");
	    } else {
		out.println("Unknown Version");
	    }
	    out.close();
	...
	try {
		...
		if (cdb.isAuthenticated()) {
		    ...
		    cloudDB.User userObj = cdb.getUser();
		    JSONObject userJObj = new JSONObject();
		    userJObj.put("user_id", userObj.getUserID().toString());
		    String[] userDeviceTokens = Auth.extractToken(token);
		    String bigDeviceToken = Auth.addUserPaddingToDeviceToken(userDeviceTokens[0], 
			    userDeviceTokens[1]);
		    userJObj.put("push_token", userObj.getDevice(bigDeviceToken).getPushToken());
		    userJObj.put("device_uid", userObj.getDevice(bigDeviceToken).getDeviceUID().toString());
		    userJObj.put("is_active", userObj.isActive());
		    userJObj.put("tenant_id", userObj.getTenantID());
		    userJObj.put("database_id", userObj.getDatabaseID());
		    userJObj.put("device_id", userObj.getDevice(bigDeviceToken).getDeviceID());
