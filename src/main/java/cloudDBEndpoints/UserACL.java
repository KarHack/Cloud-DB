 ...
 * @author karan
 */
public class UserACL extends HttpServlet {
	...
	try {
		...
		if (cdb.isAuthenticated()) {
			...
			HashMap<String, short[]> tableACL = cdb
				.getTableCRUD(UUID.fromString(request.getParameter("user_id")));
			// Extract the Table ACL from the Hashmap.
			Iterator<String> tableIt = tableACL.keySet().iterator();
			JSONArray tableACLJArr = new JSONArray();
			while (tableIt.hasNext()) {
			    // Extract the data.
			    String tableName = tableIt.next();
			    short[] tableCRUD = tableACL.get(tableName);
			    JSONObject tableACLJObj = new JSONObject();
			    tableACLJObj.put("table", tableName);
			    tableACLJObj.put("read", tableCRUD[0]);
			    tableACLJObj.put("write", tableCRUD[1]);
			    tableACLJObj.put("edit", tableCRUD[2]);
			    tableACLJObj.put("remove", tableCRUD[3]);
			    // Add the JSON Object into the JSON Array.
			    tableACLJArr.add(tableACLJObj);
			}
			// Add the table ACL to the Response JSON Object.
			respObj.put("success", tableACLJArr);
			cdb.close();
			return respObj;
		    } else {
	...
	try {
		...
		if (cdb.isAuthenticated()) {
		    ...
		    if (cdb.isServerUser()) {
			// It is a server user.
			boolean granted = cdb.grantTableCRUD(
	...
	try {
		...
		if (cdb.isAuthenticated()) {
		    ...
		    if (cdb.isServerUser()) {
			// It is a server user.
			boolean updatedGrant = cdb.updateGrantTableCRUD(
				UUID.fromString(request.getParameter("user_id")),
				request.getParameter("table_id"),
				Boolean.parseBoolean(request.getParameter("read")) ? (short) 1 : (short) 0,
				Boolean.parseBoolean(request.getParameter("write")) ? (short) 1 : (short) 0,
	...
	try {
		...
		if (cdb.isAuthenticated()) {
		    ...
		    if (cdb.isServerUser()) {
			// It is a server user.
			boolean revoked = cdb.revokeGrantTableCRUD(
				UUID.fromString(request.getParameter("user_id")),
				request.getParameter("table_id"));
