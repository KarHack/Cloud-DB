            ...
            if (cdb == null) {
                ...
                if (cdb.isAuthenticated()) {
                    ...
                    try {
                        ...
                        // Set the Select to get the Sum of the Columns of the Tables.
                        if (request.getParameterMap().containsKey("columns")) {
                            // Lets Add the Columns to be summed.
                            JSONArray columnJArr = (JSONArray) new JSONParser().parse(request.getParameter("columns"));
                            for (int i = 0; i < columnJArr.size(); i++) {
                                select.sum(columnJArr.get(i).toString());
                            }
...
//                                        result.next();
                                        ...
                                        respObj.put("success", res.getJSONResult().get(0));
                                    } else {
                            ...
                            respObj.put("error", "Please add Columns to be Summed");
                            cdb.close();
                            return respObj;
                        }
                    } catch (Exception e) {
