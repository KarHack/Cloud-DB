 ...
 * @author karan
 */
public class UserGroupRLS extends HttpServlet {
	...
	try {
		...
		if (cdb.isAuthenticated()) {
			...
			HashMap<Long, short[]> groupRLS = cdb
				.getGroupRLS(UUID.fromString(request.getParameter("user_id")));
			// Extract the Table ACL from the Hashmap.
			Iterator<Long> groupIt = groupRLS.keySet().iterator();
			JSONArray groupRLSJArr = new JSONArray();
			while (groupIt.hasNext()) {
			    // Extract the data.
			    long groupID = groupIt.next();
			    short[] roleCRUD = groupRLS.get(groupID);
			    JSONObject groupRLSJObj = new JSONObject();
			    groupRLSJObj.put("group_id", groupID);
			    groupRLSJObj.put("read", roleCRUD[0]);
			    groupRLSJObj.put("write", roleCRUD[1]);
			    groupRLSJObj.put("edit", roleCRUD[2]);
			    groupRLSJObj.put("remove", roleCRUD[3]);
			    // Add the JSON Object into the JSON Array.
			    groupRLSJArr.add(groupRLSJObj);
			}
			// Add the table ACL to the Response JSON Object.
			respObj.put("success", groupRLSJArr);
			cdb.close();
			return respObj;
		    } else {
	...
	try {
		...
		if (cdb.isAuthenticated()) {
		    ...
		    if (cdb.isServerUser()) {
			// It is a server user.
			boolean granted = cdb.grantGroupRLS(
				UUID.fromString(request.getParameter("user_id")),
				Long.parseLong(request.getParameter("group_id")),
			...
			respObj.put(granted ? "success" : "error", granted ? true : "Already Granted");
			cdb.close();
			return respObj;
		    } else {
			// It is a normal user.
	...
	try {
		...
		if (cdb.isAuthenticated()) {
		    ...
		    if (cdb.isServerUser()) {
			// It is a server user.
			boolean updatedGrant = cdb.updateGrantGroupRLS(
				UUID.fromString(request.getParameter("user_id")),
				Long.parseLong(request.getParameter("group_id")),
				Boolean.parseBoolean(request.getParameter("read")) ? (short) 1 : (short) 0,
				Boolean.parseBoolean(request.getParameter("write")) ? (short) 1 : (short) 0,
	...
	try {
		...
		if (cdb.isAuthenticated()) {
		    ...
		    if (cdb.isServerUser()) {
			// It is a server user.
			boolean revoked = cdb.revokeGrantGroupRLS(
				UUID.fromString(request.getParameter("user_id")),
				Long.parseLong(request.getParameter("group_id")));
